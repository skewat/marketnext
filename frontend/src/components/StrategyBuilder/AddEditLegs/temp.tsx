...
const AddEditLegs = ({ rows, expiries, selectedExpiry, onExpiryChange, strikePriceATM, onDrawerClose }: AddEditLegsProps) => {
  ...
  // Scroll to ATM strike when component mounts or when strikes/expiry changes
  React.useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const atm = container.querySelector<HTMLTableRowElement>('tr[data-atm="true"]');
    if (atm) {
      atm.scrollIntoView({ behavior: 'auto', block: 'center' });
    }
  }, [selectedExpiry, rows]);
  ...
  <Box sx={{ display: "flex", maxHeight: "calc(100dvh - 121px)" }} ref={containerRef}>
        <TableContainer 
          component={Paper}
          sx={{ height: "100%" }}
        >
              ...
              {rows.map((row) => {
                ...
                return (
                  <OptionTableRow 
                    ...
                    isATM={row.strike === strikePriceATM}
                  />
                );
              })}
            </TableBody>
